...
def create_avl_datastructure(geometry,cofiguration,conditions):
    """ \deprecated
        SUAVE.Methods.Aerodynamics.AVL.create_avl_datastructure(geometry,conditions)
        Isolates the parts of the vehicle geometry/conditions that AVL needs and structures
        them to match the input structure for AVL
        Does not run (undefined variable "settings")
        
        Inputs:
            geometry - geometry of vehicle
            conditions - conditions of system around geometry
            
        Outputs:
            avl_inputs - data structure holding all inputs for AVL
    """
...
def translate_avl_geometry(geometry):
    """ \deprecated
        ...
        Redefines vehicle geometry in terms of Sections as in AVL
...
def translate_avl_wing(suave_wing):
	""" SUAVE.Methods.Aerodynamics.AVL.create_avl_datastructure.translate_avl_wing(suave_wing)
        Redefines wing geometry in terms of Sections as in AVL.  
        Copies general information directly from suave wing,
        builds sections based on info from suave wing
        
        Inputs:
            suave_wing - SUAVE wing geometry
...
def translate_avl_body(suave_body):
	""" SUAVE.Methods.Aerodynamics.AVL.create_avl_datastructure.translate_avl_body(suave_body)
        Redefines body geometry in terms of Sections as in AVL
        
        Inputs:
            suave_body - SUAVE body geometry
...
def populate_wing_sections(avl_wing,suave_wing):
    """ SUAVE.Methods.Aerodynamics.AVL.create_avl_datastructure.populate_wing_sections(avl_wing,suave_wing)
        Initializes wing sections in avl wing based on info from suave wing
        
        Inputs:
            avl_wing - wing for AVL
            suave_wing - SUAVE wing geometry
...
def populate_body_sections(avl_body,suave_body):
    """ SUAVE.Methods.Aerodynamics.AVL.create_avl_datastructure.populate_body_sections(avl_body,suave_body)
        Initializes body sections in avl body based on info from suave body
        
        Inputs:
            avl_body - body for AVL
            suave_body - SUAVE body geometry
...
def translate_avl_configuration(geometry,conditions):
    """ \deprecated
        SUAVE.Methods.Aerodynamics.AVL.create_avl_datastructure.translate_avl_configuration(geometry,conditions)
        Create a configuration based on vehicle geometry
        
        Inputs:
            geometry - vehicle goemetry
            conditions - conditions of system
            
        Outputs:
            config - configurations object for AVL
    """
...
def translate_avl_cases(conditions,suave_cases):
	""" \deprecated
        SUAVE.Methods.Aerodynamics.AVL.create_avl_datastructure.translate_avl_cases(conditions,suave_cases)
        Create a configuration based on vehicle geometry
...
def setup_test_cases(conditions):
	""" \deprecated
        SUAVE.Methods.Aerodynamics.AVL.create_avl_datastructure.setup_test_cases(conditions)
        Create test cases
        ...
        Outputs:
            runcases - container of test cases
    """
	runcases = Run_Case.Container()
